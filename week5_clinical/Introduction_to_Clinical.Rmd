...
```{r}
write.csv(clinic, "/home1/aditis54/490_cluster/analysis_data/brca_clinical_data.csv", row.names=FALSE)
```

To read the csv back into your environment as a dataframe:

```{r}
clinic <- read.csv("/home1/aditis54/490_cluster/analysis_data/brca_clinical_data.csv")
```
...
#2. The data type of $gender is chr
#3. The age in years of the 5th patient is 50
#4. The 1st patient at their last follow up was 65 years. Since the age at diagnosis was 55 and the last contact days was 3767, dividing 3767 by 365 we get approximately 10 years. 
...
```{r Exercise 4.1}
# create plot here
plot(clinic$age_at_diagnosis, clinic$lymph_nodes_examined_count, xlab = "Age at Diagnosis", ylab = "# Lymph Nodes Examined", main = "# of Lymph Nodes Examined vs. Age at Patient Diagnosis")
```
...
boxplot(clinic$lymph_nodes_examined_he_count ~ clinic$gender,
        xlab = "Gender", 
        ylab = "Lymph Nodes Examined Count through Hematoxylin and Eosin (H&E)", 
        main = "Lymph Nodes Examined Count (HE) by Gender")
```
...
```{r}
no_middle_clinic <- clinic$age_at_diagnosis[under30_over60_mask]
# print(no_middle_clinic)
```
...
print(cleaned_clinic$age_category)
```
...
print(cleaned_clinic$death_event)
```
...
Middle age group has the greatest survival rate.
# 4. What age group appears to have the worst rate of survival? 
Young age group has the worst survival rate. 
# 5. What is the approx. 15-year survival rate for "Old" breast cancer patients?
The approx 15 year survival rate for "Old" breast cancer patients is 45%.
...
The p value is 0.2. Since the value is greater than 0.05, this is not statistically significant and means that there is not enough evidence to conclude that there is a significant difference in survival rates among the different age groups (Young, Middle, and Old).
